...
This reference implementation is provided for **educational and
illustrative purposes only**. It is intended to demonstrate one
possible way to apply the SCI for AI specification to LLM inference
workloads.

**Not for production use.** This project is a working prototype, not a
production-grade application. Real deployments require independent
evaluation against the team's own operational, security, and
methodology requirements.

**Vendor-neutral, not an endorsement.** The implementation uses
[EcoLogits](https://ecologits.ai/) — an open-source instrumentation
library — to obtain measured per-call energy figures from supported
provider SDKs. EcoLogits is one example of an instrumentation tool
that fits this use case at the time of writing; its inclusion here is
**not an endorsement** by the Green Software Foundation, nor a
recommendation over other measurement approaches. Alternative
instrumentation tools, measurement providers, and energy-estimation
methodologies exist, and contributors are welcome to build
reference implementations using different tooling. The provider layer
in this implementation is designed to be substitutable.

**Numbers are illustrative.** The figures in this README and in the
committed fixtures depend on the measurement tool, the assumptions in
its model, the model profiles available at the time of recording, and
the operational context. They are not authoritative carbon claims
about any specific provider, model, or workload. The intent of this
implementation is to make it easier to establish a baseline and
improve it iteratively, not to produce definitive measurements.

**Community-driven.** Alternative implementations of SCI for AI for
LLM inference — using different measurement tools, different
functional unit choices, or different languages — are explicitly
welcome alongside this one. The methodology is the standard; the
implementations are where community experience accumulates.
...
- Use **measured** energy via an instrumentation library when the
  provider SDK supports it, and a calibrated **estimated** fallback
  otherwise — with each number labelled so it's always clear which
  is which. The current implementation uses
  [EcoLogits][ecologits] as the instrumentation library; alternative
  measurement libraries can be substituted by adapting the provider
  layer.
...
Example output (real recording from a `gpt-4o-mini` run,
...
format conversion?
Energy source: measured (baseline) / measured (optimized) — measured via instrumentation library
...
real measured energy (via the instrumentation library this
implementation currently uses). Fixtures live in `docs/fixtures/` and
...
- **`gpt-5.4` measures roughly 100× the per-request emissions of
  ...
  reasoning tokens, and how the instrumentation library models
  per-token energy for larger architectures. The ratio is in the same
  order of magnitude as the cost ratio (~90×).
- **All three runs report `measured` energy from the instrumentation
  library used in this implementation.** The measurement path is the
  same for each row, so within this table the comparison is consistent
  across models. Switching to a different instrumentation library
  could shift the absolute values; this implementation makes the
  source of every figure transparent.
...
To see real numbers from an actual OpenAI run with measured energy,
...
instrumentation library`. Three fixtures are committed
(`gpt-4o-mini`, `gpt-4o`, `gpt-5.4`); replay any of them to verify
...
- **E** — energy (kWh) measured via an instrumentation library or estimated (model-card)
...
This implementation labels every reported number as **measured** or
**estimated** so the source is transparent.

**Estimated.** By default, energy comes from a per-model kWh-per-token
table (`src/sci_for_ai/energy.py`). The figures are calibrated against
real instrumentation measurements and serve as a working starting point
when measured data is not available.

**Measured.** When an instrumentation library is installed and the
provider SDK is supported, the implementation reads measured per-call
energy and embodied figures directly from the SDK's instrumented
response. The current implementation wires up
[EcoLogits](https://ecologits.ai/) for this purpose — an open-source
instrumentation library that covers several major LLM provider SDKs.
The provider layer is designed so that an alternative measurement
library could be substituted by adapting it.

```bash
# Install the current implementation's measurement library, if you
# want measured energy figures for supported providers:
pip install ecologits   # already included with [anthropic], [gemini], [openai]
```

At the time of writing, EcoLogits 0.10+ has profiles for several major
LLM provider SDKs including Anthropic, OpenAI, Google Gemini, Cohere,
Mistral AI, and Hugging Face. Model and provider coverage in any
specific instrumentation library will evolve over time independently
...
For measured per-call energy, this implementation currently integrates
with [EcoLogits](https://ecologits.ai/), an open-source instrumentation
library.
